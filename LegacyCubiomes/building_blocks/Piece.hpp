...
public:
    ...
    Piece() : orientation(), type(PT_NONE), depth(0), additionalData(0) {}

    Piece(const BoundingBox boundingBox, const FACING orientation)
        : BoundingBox(boundingBox), orientation(orientation), type(PT_NONE), depth(0), additionalData(0) {}

    Piece(c_int minX, c_int minY, c_int minZ, c_int maxX, c_int maxY, c_int maxZ,
          const FACING orientation)
        : BoundingBox(minX, minY, minZ, maxX, maxY, maxZ), orientation(orientation), type(PT_NONE), depth(0),
          additionalData(0) {}
