...
std::vector<float> getFracs(int* biomes)
{
    ...
    while (index < biomesSize) {
        biomeGroup = getBiomeGroup(biomes[index]); // getting the biome group it is in (take the biomeID and assign it a catagory)
        fracs[biomeGroup]++;
        index++;
    }
    for (int div = 0; div < 8; div++) {
        fracs[div] /= biomesSize;
    }
    return fracs;
}
...
bool getIsMatch(int* biomes)
{
    ...
    if (floatFrac[7] < 0.001F) {
        return false;
    }
    for (int i = 0; i < 5; i++) {
        if (floatFrac[i] < 0.01F) {
            return false;
        }
    }
    if (0.25F < floatFrac[5]) {
        return false;
    }
    if (0.1F < floatFrac[6]) {
        return false;
