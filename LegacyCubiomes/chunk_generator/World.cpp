...
void World::decorateChunkAndNeighbors(const Pos2D &pos, int radius) {
    // First, load all chunks within the radius
    ...
    std::cout << "Decorating chunks" << std::endl;
    for (int dx = -radius - 1; dx <= radius + 1; ++dx) {
        for (int dz = -radius - 1; dz <= radius + 1; ++dz) {
            Pos2D neighborPos = pos + Pos2D(dx, dz);
            Chunk::populate(*g, neighborPos.x, neighborPos.z, this);
        }
    }
}

int World::getBlockId(int x, int y, int z) const {
    if (const ChunkPrimer *chunk = getChunk({x >> 4, z >> 4})) {
        return chunk->getBlockId(x & 15, y, z & 15);
    }
    return 0;
}

int World::getBlockId(const Pos3D &pos) const {
    return this->getBlockId(pos.getX(), pos.getY(), pos.getZ());
}

const lce::blocks::Block *World::getBlock(int x, int y, int z) const {
    if (const ChunkPrimer *chunk = getChunk({x >> 4, z >> 4})) {
...
const lce::blocks::Block *World::getBlock(const Pos3D &pos) const {
...
bool World::isAirBlock(int x, int y, int z) const {
    if (ChunkPrimer *chunk = getChunk({x >> 4, z >> 4})) {
        return chunk->isAirBlock(x & 15, y, z & 15);
    }
    return false;
}

bool World::isAirBlock(const Pos3D &pos) const {
...
Pos3D World::getHeight(const Pos3D &pos) const {
    return {pos.getX(), this->getHeight(pos.getX(), pos.getZ()), pos.getZ()};
}

int World::getTopSolidOrLiquidBlock(int x, int z) const {
    Pos2D chunkPos = {x >> 4, z >> 4};
    ChunkPrimer *chunk = getChunk(chunkPos);
    if (chunk) {
...
Pos3D World::getTopSolidOrLiquidBlock(const Pos3D &pos) const {
    return {pos.getX(), this->getTopSolidOrLiquidBlock(pos.getX(), pos.getZ()), pos.getZ()};
}

int World::getPrecipitationHeight(c_int x, c_int z) const {
    ChunkPrimer *chunk = getChunk({x >> 4, z >> 4});
    if (chunk) {
...
bool World::canBlockFreeze(const Pos3D &pos, bool noWaterAdj) const {
...
ND bool World::canBlockFreezeWater(const Pos3D &pos) const {
    return this->canBlockFreeze(pos, false);
}

bool World::canSnowAt(const Pos3D &pos, bool checkLight) const {
    if (Biome::getBiomeForId(g->getBiomeAt(1, pos.getX(), pos.getZ()))->getFloatTemperature(pos) >= 0.15F) {
        return false;
    }
