...
public:
    ...
    Biome(std::string biomeNameIn, float baseHeightIn, float heightVariationIn, bool enableSnowIn, float temperatureIn) : biomeName(biomeNameIn), baseHeight(baseHeightIn), heightVariation(heightVariationIn), temperature(temperatureIn), enableSnow(enableSnowIn) {
        setTemperatureNoise();
    }
    Biome(std::string biomeNameIn, float baseHeightIn, float heightVariationIn, bool enableSnowIn, float temperatureIn, Block topBlockIn) : biomeName(biomeNameIn), baseHeight(baseHeightIn), heightVariation(heightVariationIn), temperature(temperatureIn), enableSnow(enableSnowIn), topBlock(topBlockIn) {
        setTemperatureNoise();
    }
    Biome(std::string biomeNameIn, float baseHeightIn, float heightVariationIn, bool enableSnowIn, float temperatureIn, Block topBlockIn, Block fillerBlockIn) : biomeName(biomeNameIn), baseHeight(baseHeightIn), heightVariation(heightVariationIn), temperature(temperatureIn), enableSnow(enableSnowIn), topBlock(topBlockIn), fillerBlock(fillerBlockIn) {
        setTemperatureNoise();
    }
    virtual ~Biome() {}
    ...
    virtual void genTerrainBlocks(int64_t worldSeed, uint64_t* rand, ChunkPrimer *chunkPrimerIn, int x, int z, double noiseVal) {
...
public:
    BiomeDesert(std::string biomeName, float baseHeight, float heightVariation, bool enableSnow, float temperature)
        : Biome(biomeName, baseHeight, heightVariation, enableSnow, temperature, Block(Items::SAND_ID), Block(Items::SAND_ID)) {}
};
...
public:
    ...
    void genTerrainBlocks(int64_t worldSeed, uint64_t* rand, ChunkPrimer *chunkPrimerIn, int x, int z, double noiseVal) override
    {
        this->topBlock.block = Items::GRASS_ID;
        this->fillerBlock.block = Items::DIRT_ID;

        if ((noiseVal < -1.0 || noiseVal > 2.0) && (this->biomeName == "Extreme_Hills_M" || this->biomeName == "Extreme_Hills_Plus_M"))
        {
            this->topBlock.block = Items::GRAVEL_ID;
            this->fillerBlock.block = Items::GRAVEL_ID;
        }
        else if (noiseVal > 1.0 && (this->biomeName == "Extreme_Hills_M" || this->biomeName == "Extreme_Hills_Plus_M" || this->biomeName == "Extreme_Hills"))
        {
...
public:
    BiomeForestMutated(std::string biomeName, float baseHeight, float heightVariation, bool enableSnow, float temperature)
        : BiomeForest(biomeName, baseHeight, heightVariation, enableSnow, temperature) {}
};

class BiomeTaiga : public Biome {
public:
    int type;
    BiomeTaiga(std::string biomeName, float baseHeight, float heightVariation, bool enableSnow, float temperature, int type)
        : Biome(biomeName, baseHeight, heightVariation, enableSnow, temperature), type(type) {}
...
public:
    bool superIcy;
    BiomeSnow(bool superIcy, std::string biomeName, float baseHeight, float heightVariation, bool enableSnow, float temperature)
        : Biome(biomeName, baseHeight, heightVariation, enableSnow, temperature)
    {
...
public:
    BiomeMushroomIsland(std::string biomeName, float baseHeight, float heightVariation, bool enableSnow, float temperature)
        : Biome(biomeName, baseHeight, heightVariation, enableSnow, temperature, Block(Items::MYCELIUM_ID)) {}
};
...
public:
    BiomeStoneBeach(std::string biomeName, float baseHeight, float heightVariation, bool enableSnow, float temperature)
        : Biome(biomeName, baseHeight, heightVariation, enableSnow, temperature, Block(Items::STONE_ID), Block(Items::STONE_ID)) {}
};
...
public:
    ...
    BiomeMesa(bool hasBrycePillars, bool hasForest, std::string biomeName, float baseHeight, float heightVariation, bool enableSnow, float temperature)
        : Biome(biomeName, baseHeight, heightVariation, enableSnow, temperature), worldSeed(0), brycePillars(hasBrycePillars), hasForest(hasForest) {}
...
public:
    BiomeSavannaMutated(std::string biomeName, float baseHeight, float heightVariation, bool enableSnow, float temperature)
        : Biome(biomeName, baseHeight, heightVariation, enableSnow, temperature) {}
