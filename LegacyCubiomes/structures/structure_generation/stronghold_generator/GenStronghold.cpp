...
namespace stronghold_generator {
    ...
    void StrongholdGenerator::generate(int64_t worldSeed, int chunkX, int chunkZ) {
        ...
        do {
            ...
            while (this->pendingPiecesSize != 0) {
                ...
                if (this->portalRoomPiece != nullptr && this->generationStopped &&
                    this->generationStep == GenerationStep::LAYOUT) {
                    return;
                }
            }
    ...
    BoundingBox
    StrongholdGenerator::createPieceBoundingBox(PieceType pieceType, int x, int y, int z,
                                                                      DIRECTION direction) {
        switch (pieceType) {
    ...
    Piece
    StrongholdGenerator::createPiece(PieceType pieceType, uint64_t *random,
    ...
    bool StrongholdGenerator::generatePieceFromSmallDoor(uint64_t *random, int x, int y, int z,
        ...
        for (int attempt = 0; attempt < 5; attempt++) {
            ...
            for (int piecePlaceCountIndex = 0;
                 piecePlaceCountIndex < this->piecePlaceCountsSize; piecePlaceCountIndex++) {
                PiecePlaceCount &piecePlaceCount = this->piecePlaceCounts[piecePlaceCountIndex];
    ...
    void StrongholdGenerator::generateAndAddPiece(uint64_t *random, int x, int y, int z,
                                                                        DIRECTION direction, int depth) {
        if (depth > 50) {
            return;
        }
    ...
    void StrongholdGenerator::generateSmallDoorChildForward(Piece &piece, uint64_t *random, int n,
                                                                                  int n2) {
    ...
    void StrongholdGenerator::generateSmallDoorChildLeft(Piece &piece, uint64_t *random, int n,
                                                                               int n2) {
        DIRECTION direction = piece.orientation;
        switch (direction) {
            case DIRECTION::SOUTH:
            case DIRECTION::NORTH: {
                return this->generateAndAddPiece(random, piece.boundingBox.minX - 1, piece.boundingBox.minY + n,
                                                 piece.boundingBox.minZ + n2, DIRECTION::WEST, piece.depth);
            }
            case DIRECTION::EAST:
            case DIRECTION::WEST: {
    ...
    void StrongholdGenerator::generateSmallDoorChildRight(Piece &piece, uint64_t *random, int n,
                                                                                int n2) {
        DIRECTION direction = piece.orientation;
        switch (direction) {
            case DIRECTION::SOUTH:
    ...
    void StrongholdGenerator::addChildren(Piece &piece, uint64_t *random) {
        switch (piece.type) {
            case PieceType::STRAIGHT: {
    ...
    Piece *
    StrongholdGenerator::findCollisionPiece(BoundingBox &boundingBox) {
