...
namespace structure_rolls {
    ...
    public:
        ...
        MU static bool generateStructure(ChunkPrimer *chunk, generation::Stronghold *strongholdGenerator, uint64_t *random, const Piece& pieceStop, int chestXChunk, int chestZChunk);
    };

    template<bool isStrongholdChest>
    bool Stronghold::generateStructure(ChunkPrimer *chunk, generation::Stronghold *strongholdGenerator, uint64_t *random, const Piece& pieceStop, int chestXChunk, int chestZChunk) {
        ...
        for (int pieceIndex = 0; pieceIndex < strongholdGenerator->pieceArraySize; ++pieceIndex) {
            ...
            if (piece.type != generation::Stronghold::PieceType::NONE) {
                if (piece.intersects(chunkBoundingBox)) {
                    if(chunk && piece.type != generation::Stronghold::PieceType::PORTAL_ROOM && isLiquidInStructureBoundingBox(chunkBoundingBox, piece, chunk))
                        continue;
