...
namespace structure_rolls {
    ...
    void Mineshaft::generateStructure(const generation::Mineshaft* mineshaftGenerator,
        ...
        for (int pieceIndex = 0; pieceIndex < mineshaftGenerator->pieceArraySize; ++pieceIndex) {
            ...
            if (piece.type != static_cast<i8>(generation::Mineshaft::PieceType::NONE)) {
                c_auto chunkBoundingBox =
                ...
                if (const BoundingBox& pieceBoundingBox = piece; pieceBoundingBox.intersects(chunkBoundingBox)) {
                    if (piece.type == static_cast<i8>(generation::Mineshaft::PieceType::CORRIDOR)) {
    ...
    void Mineshaft::generateChest(const ChunkPrimer* chunk, const BoundingBox& chunkBB,
                                  const Piece& piece, RNG& rng, c_int x, c_int y, c_int z) {
    ...
    void Mineshaft::placeCobWeb(const ChunkPrimer* chunk, const BoundingBox& chunkBB,
                                const Piece& piece, RNG& rng, c_int x, c_int z) {
        c_int xPos = piece.getWorldX(x, z);
