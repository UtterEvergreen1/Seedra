...
bool WorldGenDungeons::generate(World* worldIn, RNG& rng, const Pos3D &position) const {
    ...
    int i = 3;
    int j = rng.nextInt(2) + 2;
    int k = -j - 1;
    int l = j + 1;
    int i1 = -1;
    int j1 = 4;
    int k1 = rng.nextInt(2) + 2;
    int l1 = -k1 - 1;
    int i2 = k1 + 1;
    int j2 = 0;
    ...
    for (int k2 = k; k2 <= l; ++k2) {
        for (int l2 = -1; l2 <= 4; ++l2) {
            for (int i3 = l1; i3 <= i2; ++i3) {
                ...
                bool flag = ids::isSolidBlock(worldIn->getBlockId(blockpos));
    ...
    if (j2 >= 1 && j2 <= 5) {
            ...
            for (int i4 = 3; i4 >= -1; --i4) {
                for (int k4 = l1; k4 <= i2; ++k4) {
                    ...
                    if (k3 != k && i4 != -1 && k4 != l1 && k3 != l && i4 != 4 && k4 != i2) {
                        ...
                        worldIn->setBlock(blockpos1, ids::AIR_ID);
                    } else if (ids::isSolidBlock(worldIn->getBlockId(blockpos1)) && worldIn->getBlockId(blockpos1) != ids::CHEST_ID) {
        ...
        for (int l3 = 0; l3 < 2; ++l3) {
            for (int j4 = 0; j4 < 3; ++j4) {
                int l4 = position.getX() + rng.nextInt(j * 2 + 1) - j;
                int i5 = position.getY();
                int j5 = position.getZ() + rng.nextInt(k1 * 2 + 1) - k1;
                Pos3D blockpos2(l4, i5, j5);

                if (worldIn->isAirBlock(blockpos2)) {
                    int j3 = 0;

                    for (auto enumfacing : FACING_HORIZONTAL) {
                        if (ids::isSolidBlock(worldIn->getBlockId(blockpos2.offset(enumfacing)))) {
                            ++j3;
                        }
                    }
...
void WorldGenDungeons::pickMobSpawner(RNG &rng) const {
    //return SPAWNERTYPES[rng.nextInt(sizeof(SPAWNERTYPES))];
    rng.nextInt(sizeof(SPAWNERTYPES));
}