...
bool BlockCactus::canBlockStay(const ChunkPrimer *worldIn, const Pos3D &pos) {
...
bool WorldGenCactus::generate(ChunkPrimer *worldIn, RNG &rng, const Pos3D &pos) const {
    ...
    for (int i = 10; i != 0; --i)
    {
        ...
        if (worldIn->isAirBlock(blockPos))
        {
            ...
            for (int k = 0; k < j; ++k) {
                ...
                {
                    worldIn->setBlockId(blockPos.up(iVar1), lce::blocks::ids::CACTUS_ID);
                }
                iVar1 = iVar1 + 1;
            }
        }
