...
bool BlockBush::canPlaceBlockAt(const ChunkPrimer* chunk, const Pos3D& pos) {
...
bool BlockBush::canBlockStay(const ChunkPrimer *chunk, const Pos3D &pos) {
    return BlockBush::canSustainBush(chunk->getBlockId(pos.down()));
}

bool WorldGenBush::generate(ChunkPrimer *worldIn, RNG &rng, const Pos3D &pos) const {
    for (int i = 0; i < 64; ++i)
    {
