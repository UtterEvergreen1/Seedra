...
bool WorldGenTrees::generate(World *worldIn, RNG &rng, const Pos3D &pos) const {
    ...
    if (pos.getY() >= 1 && pos.getY() + height <= 255) {
        ...
        if ((belowBlockId == ids::GRASS_ID || belowBlockId == ids::DIRT_ID || belowBlockId == ids::FARMLAND_ID)
            ...
            //placeTrunk
            if (!placeTrunk(worldIn, rng, pos, height, this->metaWood, this->vinesGrow))
                return true;
            int startY = pos.getY();
            int topY = startY + height;
            int leafStartY = startY - 3 + height;
            for (int y = topY; y >= leafStartY; --y) {
                ...
                for (int x = pos.getX() - radius; x <= pos.getX() + radius; ++x) {
                    for (int z = pos.getZ() - radius; z <= pos.getZ() + radius; ++z) {
                        if (std::abs(x - pos.getX()) != radius || std::abs(z - pos.getZ()) != radius || (rng.nextInt(2) != 0 && yOffset != 0)) {
