...
bool WorldGenFlowers::generate(ChunkPrimer* worldIn, RNG& rng, const Pos3D& position) const {
    for (int i = 0; i < 64; ++i) {
        ...
        if (worldIn->isAirBlock(blockPos) && BlockBush::canBlockStay(worldIn, blockPos)) {
            worldIn->setBlockAndData(blockPos, this->flowerId, (int)this->flowerType);
        }
    }
    return true;
}
