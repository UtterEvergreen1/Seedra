...
bool WorldGenBlockBlob::generate(World * worldIn, RNG &rng, const Pos3D &pos) const {
    ...
    while (true) {
        if (position.getY() > 3)
        {
            ...
            if (blockId != ids::GRASS_ID && blockId != ids::DIRT_ID && blockId != ids::STONE_ID)
            {
                goto label50;
            }
        }
        ...
        for (int i = 0; i < 3; ++i)
        {
            int j = rad + rng.nextInt(2);
            int k = rad + rng.nextInt(2);
            int l = rad + rng.nextInt(2);
            ...
            position = position.add(-(rad + 1) + rng.nextInt(2 + rad * 2), 0 - rng.nextInt(2), -(rad + 1) + rng.nextInt(2 + rad * 2));
        }
