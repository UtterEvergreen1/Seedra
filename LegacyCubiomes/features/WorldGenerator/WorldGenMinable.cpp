...
bool WorldGenMinable::generate(ChunkPrimer* chunk, RNG& rng, const Pos3D& pos) const {
    ...
    for (int i = 0; i <= this->blockCount; ++i)
    {
        ...
        double d10 = (double)((MathHelper::sin(3.1415927F * f1) + 1.0F) * d9 + 1.0) / 2.0;
                    ...
                    if (d12 * d12 + d13 * d13 < 1.0)
                    {
                        for (int j2 = l; j2 <= k1; ++j2)
                        {
                            ...
                            if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0)
                            {
                                ...
                                if (chunk->getBlockId(blockpos) == lce::blocks::ids::STONE_ID)
                                {
                                    chunk->setBlock(blockpos, this->oreBlock);
                                }
                            }
                        }
                    }
