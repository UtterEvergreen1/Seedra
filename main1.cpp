...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    std::sort(pieceVector.begin(), pieceVector.end(), [](const Piece& a, const Piece& b) {
    ...
    for (auto sortedPiece : pieceVector) {
        ...
        bool status = picture.drawBox(
                sortedPiece.minX - minX,
                sortedPiece.minZ - minZ,
                sortedPiece.maxX - minX,
                sortedPiece.maxZ - minZ,
                rgb[0], rgb[1], rgb[2]
        );
        if (!status)
    ...
    picture.saveWithName("stronghold" + std::to_string(pos.x) +
        "_" + std::to_string(pos.z) + ".png", path);
