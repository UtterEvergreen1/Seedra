...
namespace finders {
    ...
    public:
        ...
        explicit LargeCaveFinder(c_i64 _worldSeed) : worldSeed(_worldSeed) {}

        static std::vector<LargeCaveResult> findAllLargeCaves(c_i64 worldSeed, AreaRange& range) {
            ...
            for (int x = lower.x; x <= upper.x; x++) {
                ...
                for (int z = lower.z; z <= upper.z; z++) {
                    ...
                    caveGen.rng.setSeed(adjustedX ^ adjustedZ ^ worldSeed);
        ...
        void addFeature(const Pos2D baseChunk, std::vector<LargeCaveResult>& results) {
            ...
            for (int currentTunnel = 0; currentTunnel < tunnelCount; ++currentTunnel) {
                DoublePos3D tStart;
                tStart.x = static_cast<double>(baseChunk.x * 16 + rng.nextInt<16>());
                tStart.y = static_cast<double>(rng.nextInt(rng.nextInt<120>() + 8));
                tStart.z = static_cast<double>(baseChunk.z * 16 + rng.nextInt<16>());

                int segmentCount = 1;

                if (rng.nextInt<4>() == 0) {
                    c_i64 seedModifier = rng.nextLongI();
                ...
                for (int currentSegment = 0; currentSegment < segmentCount; ++currentSegment) {
                    c_float tYaw = rng.nextFloat() * (PI_FLOAT * 2.0F);
                    c_float tPitch = (rng.nextFloat() - 0.5F) * 2.0F / 8.0F;
                    float tWidth = rng.nextFloat() * 2.0F + rng.nextFloat();

                    if (rng.nextInt(10) == 0) { tWidth *= rng.nextFloat() * rng.nextFloat() * 3.0F + 1.0F; }

                    addTunnel(results, rng.nextLongI(), baseChunk, tStart, tWidth, tYaw, tPitch, 0, 0, 1.0);
                }
            }
        }
        ...
        void addTunnel(std::vector<LargeCaveResult>& results,
            ...
            for (; theCurrentSegment < theMaxSegment; ++theCurrentSegment) {
                ...
                if (   theStart.x + adjustedWidth < genBounds.minX
                    || theStart.x - adjustedWidth > genBounds.maxX
                    || theStart.z + adjustedWidth < genBounds.minZ
                    || theStart.z - adjustedWidth > genBounds.maxZ) {
                    continue;
                    }
                ...
                if (adjustedWidth > CUT_OFF) {
                    results.emplace_back(max - min, theStart, DoublePos2D(adjustedWidth, adjustedHeight));
                }
