...
bool WorldGenTrees::generate(World* worldIn, RNG& rng, const Pos3D& pos) const {
    ...
    if (pos.getY() >= 1 && pos.getY() + height <= 255) {
        ...
        using namespace lce::blocks;
        if ((belowBlockId == GRASS_ID || belowBlockId == DIRT_ID || belowBlockId == FARMLAND_ID) &&
...
void WorldGenTrees::addHangingVine(World* worldIn, const Pos3D& pos, const EnumFacing facing) {
    ...
    for (Pos3D blockPos = pos.down(); worldIn->getBlockId(blockPos) == AIR_ID && i > 0; --i) {
