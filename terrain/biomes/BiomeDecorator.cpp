...
#include "terrain/decorators/WorldGenDeadBush.hpp"
#include "terrain/decorators/WorldGenLiquids.hpp"
...
void BiomeDecorator::genDecorations(World *world, Biome *biome, RNG &rng) {
    ...
    if (rng.nextInt(32) == 0) {
        ...
        c_int z = rng.nextInt(16) + 8;
        c_int y = rng.nextInt(world->getHeight(this->chunkBlockPos.add(x, 0, z)).getY() * 2);
        (WorldGenCucurbitsBlock(lce::blocks::CARVED_PUMPKIN_ID, true)).generate(
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        WorldGenLiquids(lce::blocks::FLOWING_WATER_ID).generate(world, rng, this->chunkBlockPos.add(x, y, z));
    }
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        WorldGenLiquids(lce::blocks::FLOWING_LAVA_ID).generate(world, rng, this->chunkBlockPos.add(x, y, z));
    }
}
