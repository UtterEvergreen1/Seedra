...
class PopulationReverser {
    ...
    static u64 modInverse(u64 a, u64 mod = 1ULL << 16);

    static i64 getPartialAddend(i64 partialSeed, int x, int z, int bits);

    static void addWorldSeed(
        i64 firstAddend,
        ...
        int z,
        i64 chunkSeed,
        std::vector<i64> &worldSeeds
    );

public:
    static std::vector<i64> getSeedsFromChunkSeed(u64 chunkSeed, int x, int z);
};
