...
#include "structures/gen/stronghold/StrongholdStones.hpp"
...
bool StructureComponent::isLiquidInStructureBoundingBox(const BoundingBox& chunkBoundingBoxIn,
    ...
    for (int x = minX; x <= maxX; ++x) {
        for (int z = minZ; z <= maxZ; ++z) {
            ...
            if (lce::blocks::isLiquidBlock(chunk->getBlockId(x, maxY, z))) return true;
        }
    }
    for (int x = minX; x <= maxX; ++x) {
        for (int y = minY; y <= maxY; ++y) {
            if (lce::blocks::isLiquidBlock(chunk->getBlockId(x, y, minZ))) return true;
            if (lce::blocks::isLiquidBlock(chunk->getBlockId(x, y, maxZ))) return true;
        }
    }
    for (int z = minZ; z <= maxZ; ++z) {
        for (int y = minY; y <= maxY; ++y) {
            if (lce::blocks::isLiquidBlock(chunk->getBlockId(minX, y, z))) return true;
            if (lce::blocks::isLiquidBlock(chunk->getBlockId(maxX, y, z))) return true;
        }
    }
    return false;
}
...
bool StructureComponent::isLiquidInStructureBoundingBox(World& worldIn, const BoundingBox& bbIn) const {
    ...
    for (int x = checkMinX; x <= checkMaxX; ++x) {
        for (int z = checkMinZ; z <= checkMaxZ; ++z) {
            if (lce::blocks::isLiquidBlock(worldIn.getBlockId(x, checkMinY, z))) return true;
            if (lce::blocks::isLiquidBlock(worldIn.getBlockId(x, checkMaxY, z))) return true;
        }
    }
    for (int x = checkMinX; x <= checkMaxX; ++x) {
        for (int y = checkMinY; y <= checkMaxY; ++y) {
            if (lce::blocks::isLiquidBlock(worldIn.getBlockId(x, y, checkMinZ))) return true;
            if (lce::blocks::isLiquidBlock(worldIn.getBlockId(x, y, checkMaxZ))) return true;
        }
    }
    for (int z = checkMinZ; z <= checkMaxZ; ++z) {
        for (int y = checkMinY; y <= checkMaxY; ++y) {
            if (lce::blocks::isLiquidBlock(worldIn.getBlockId(checkMinX, y, z))) return true;
            if (lce::blocks::isLiquidBlock(worldIn.getBlockId(checkMaxX, y, z))) return true;
        }
    }
